# Routine: MIDDAY SCAN (runs ~12:30 ET, Mon–Fri)
...
- Reconcile against the **Open Positions** section of `memory/TRADE-LOG.md`. If Alpaca shows a position closed (a bracket stop or target filled since last run), record the exit.
- For each open position, note the **Tier** from `memory/TRADE-LOG.md`: `EB_PULLBACK`, `EB_MOMENTUM`, or `BULLISH`.
...
1. **Stop hit / closed by bracket?** → already out; log the exit, compute P&L, move to **Closed Trades** section of `memory/TRADE-LOG.md`.
...
- Reached **Target 1** (price at or above `T1` in `memory/TRADE-LOG.md`, or daily RSI approaching 60)?
...
- Compute the **3-bar trailing stop**: the low of the 3 most recently completed daily candles (not the live candle).
  ...
  ```
- If `3_bar_low > current_stop_in_TRADE-LOG` → raise the stop:
  `replace_stop(symbol, 3_bar_low)`, update the row in `memory/TRADE-LOG.md`, log the change.
...
### 3. Update state
- Update the **Open Positions** section of `memory/TRADE-LOG.md` (new stops, partial exits, status changes).
- For any FULL close: move the row to **Closed Trades** section with P&L, R-multiple, exit reason, and tier. Update the sector loss tracker in the **Weekly Tracker** section (increment consecutive losses on a losing trade, reset on a win).
- If a sector hits 2 consecutive losses → mark it paused in the Weekly Tracker.
- Update the `Last Updated` timestamp.
- **Skip the commit if nothing changed** — no noise in git history for quiet midday scans.

### 4. Close out
- If changes made: commit + push `memory/TRADE-LOG.md`.
